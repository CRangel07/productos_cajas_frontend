  ...
  >
    ...
    >
      <div class="flex items-center flex-wrap justify-between">
        <h2 class="mb-2 text-2xl font-semibold text-cyan-600">
          {{ isDragging ? "Tus archivos van aquí" : "Selecciona Archivo" }}
        </h2>
        <CloudUploadIcon
          :size="42"
          :class="{
...
const seleccionarLinea: Ref<"linea75" | "linea76" | "none" | "linea7576"> =
  ref("none");
...
const emit = defineEmits<{
  (
    e: "subir-txt",
    data: FormData,
    linea: "linea75" | "linea76" | "linea7576" | "none"
  ): void;
}>();
...
const validateFileContent = (content: string) => {
  ...
  if (lines.length >= 3) {
    const secondLineValid = lines[0].includes("cei-029s-2.6");
    const thirdLineValid = lines[1].includes("EXISTENCIA ACTUAL DE ARTICULOS");
    ...
    if (secondLineValid && thirdLineValid) {
      ...
      alerta({
        ...
        icon: "error",
        text: "El archivo no es el correcto, recuerda que debe ser la 'EXISTENCIA ACTUAL DE ARTICULOS'",
      });
      return [];
    }
  } else {
...
const handleSubmit = async () => {
  ...
  if (respuesta.isConfirmed) {
    if (file.value) {
      ...
      await apiFetch("/lineas", { method: "POST", body: formData });
      cleanAll();
    }
  }
};
